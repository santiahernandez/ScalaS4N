...
import week2.Inmutable.{Const, List, Nil}
...
class ListHighOrderFunctionTesting extends FunSuite{
  ...
  * */
  ...
  test("funcion dropwhile usando foldLeft"){
    val lst = List(1,2,6,3,4,5,6,7,8,9)
    val p = (x:Int)=>{
      x<5 || x>7
    }
    assert(List.dropWhileFL(lst)(p)===List(6,3,4,5,6,7,8,9))
  }
}
