...
object List {
  ...
  /*
  week2.Funciones de orden superior
  * */
  ...
  //def takewhileFL[A](lst:List[A])(p:A=>Boolean):List[A] = flip(foldLeft(flip(lst),Nil:List[A])((h,t) => if (p(t)) addEnd(h,t) else Nil))

  def takewhileFL[A](lst:List[A])(p:A=>Boolean):List[A] = foldLeft(lst,(true:Boolean, Nil:List[A]))((b,t)=>b match {
    case (true,_) => if (p(t)) (true , addEnd(b._2,t))
                      else (false,b._2)
    case (false,_) => (false,b._2)
  })._2

  def dropWhileFL[A](lst:List[A])(p:A=>Boolean):List[A] = foldLeft(lst,(true:Boolean,Nil:List[A]))((b,a)=> b match {
